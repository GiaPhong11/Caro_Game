...
function Cell(x, y,rows,columns) {
    this.x = x;
    this.y = y;
    ...
    this.getHtml = function(){
        ...
        var cellHtml = '<button id="cell-'+x+'-'+y+'" onclick="play('+x+','+y+')" class="cell" style="position: absolute;z-index:2; width: '+
        DEFAULT_CELL_SIZE+'px; height:'+
        DEFAULT_CELL_SIZE+'px; left:'+
        left+'px; top:'+
        top+'px; line-height: '+
        DEFAULT_CELL_SIZE+'px;"></button>';
        return cellHtml;
    };
...
function GameBoard(rows, cols, elementId) {
    ...
    this.draw = function () {
        ...
        for(var i = 0; i < this.rows; i++){
            ...
            for(var j = 0; j < this.cols; j++){
                ...
                row.push(cell);
               var resume = gameBoardDiv.innerHTML += cell.getHtml();
            }   
        }
        resume += '</table>';
    };
    ...
    this.endGame = function (count) {
        if(count >= 5 && this.turn == VALUE_O){
            this.isOver = true;
            alert("Player 1: " +document.getElementById("name1").value+" won!!!");
        }else if(count >= 5 && this.turn == VALUE_X){
            this.isOver = true;
            alert("Player 2: "+document.getElementById("name2").value+" won!!!");
        }
    };
}
...
function start() {
   ...
   gameBoard = new GameBoard(rows, columns, "game-board");
   gameBoard.draw();
}
var gameBoard;
start();
